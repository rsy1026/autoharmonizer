...
class DataGenerator(Sequence):
    ...
    def __getitem__(self, index):
        onehot_melody_left = to_categorical(
            self.input_melody_left[
                index * self.batch_size : (index + 1) * self.batch_size
            ],
            num_classes=128,
        )
        onehot_melody_right = to_categorical(
            self.input_melody_right[
                index * self.batch_size : (index + 1) * self.batch_size
            ],
            num_classes=128,
        )
        onehot_beat_left = to_categorical(
            self.input_beat_left[
                index * self.batch_size : (index + 1) * self.batch_size
            ],
            num_classes=5,
        )
        onehot_beat_right = to_categorical(
            self.input_beat_right[
                index * self.batch_size : (index + 1) * self.batch_size
            ],
            num_classes=5,
        )
        onehot_key_left = to_categorical(
            self.input_key_left[
                index * self.batch_size : (index + 1) * self.batch_size
            ],
            num_classes=16,
        )
        onehot_key_right = to_categorical(
            self.input_key_right[
                index * self.batch_size : (index + 1) * self.batch_size
            ],
            num_classes=16,
        )
        onehot_condition_left = np.concatenate(
            (onehot_beat_left, onehot_key_left), axis=-1
        )
        onehot_condition_right = np.concatenate(
            (onehot_beat_right, onehot_key_right), axis=-1
        )
        onehot_chord_left = to_categorical(
            self.input_chord_left[
                index * self.batch_size : (index + 1) * self.batch_size
            ],
            num_classes=self.chord_nums,
        )
        onehot_chord = to_categorical(
            self.output_chord[index * self.batch_size : (index + 1) * self.batch_size],
            num_classes=self.chord_nums,
        )
        ...
        return (
            [
                onehot_melody_left,
                onehot_melody_right,
                onehot_condition_left,
                onehot_condition_right,
                onehot_chord_left,
            ],
            onehot_chord,
        )
...
def create_training_data(
    ...
    for songs_idx in trange(len(data_corpus)):
        ...
        for song in songs:
            ...
            song_chord = (
                segment_length * [0]
                + [chord_types_dict[cho] for cho in song[3]]
                + segment_length * [0]
            )
...
def build_model(
    ...
    input_melody_left = Input(shape=(segment_length, 128), name="input_melody_left")
    melody_left = TimeDistributed(Dense(12, activation="relu"))(input_melody_left)

    input_melody_right = Input(shape=(segment_length, 128), name="input_melody_right")
    melody_right = TimeDistributed(Dense(12, activation="relu"))(input_melody_right)

    input_condition_left = Input(
        shape=(segment_length, 21), name="input_condition_left"
    )
    condition_left = TimeDistributed(Dense(5, activation="relu"))(input_condition_left)

    input_condition_right = Input(
        shape=(segment_length, 21), name="input_condition_right"
    )
    condition_right = TimeDistributed(Dense(5, activation="relu"))(
        input_condition_right
    )

    input_chord_left = Input(
        shape=(segment_length, len(chord_types)), name="input_chord_left"
    )
    chord_left = TimeDistributed(
        Dense(int(math.sqrt(len(chord_types))), activation="relu")
    )(input_chord_left)
    ...
    for idx in range(num_layers):
        ...
        if idx != num_layers - 1:
            melody_left = TimeDistributed(Dense(rnn_size, activation="relu"))(
                melody_left
            )
        ...
        if idx != num_layers - 1:
            melody_right = TimeDistributed(Dense(rnn_size, activation="relu"))(
                melody_right
            )
        ...
        if idx != num_layers - 1:
            condition_left = TimeDistributed(Dense(rnn_size, activation="relu"))(
                condition_left
            )
        condition_left = BatchNormalization()(condition_left)
        condition_left = Dropout(dropout)(condition_left, training=training)

        condition_right = LSTM(
            rnn_size,
            name="condition_right_" + str(idx + 1),
            return_sequences=return_sequences,
        )(condition_right)
        if idx != num_layers - 1:
            condition_right = TimeDistributed(Dense(rnn_size, activation="relu"))(
                condition_right
            )
    ...
    merge = concatenate(
        [melody_left, melody_right, condition_left, condition_right, chord_left]
    )
    # Creating the hidden layer of the LSTM
    for idx in range(num_layers):
        merge = Dense(units=rnn_size, activation="relu", name="merge" + str(idx))(merge)
        merge = BatchNormalization()(merge)
        merge = Dropout(dropout)(merge, training=training)

    # Create output layer
    output_chord = Dense(
        units=len(chord_types), activation="softmax", name="output_chord"
    )(merge)
    model = Model(
        inputs=[
            input_melody_left,
...
def train_model(
    ...
    if len(data_val[0]) != 0:
        ...
        history = model.fit(
            x=train_generator, epochs=epochs, verbose=verbose, callbacks=[checkpoint]
        )
